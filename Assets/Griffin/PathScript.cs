...
{
    [Tooltip("points of a bézier curve")]
    [SerializeField] public Transform[] controlPoints;

    private Vector2 objectPostition;
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            objectPostition = Mathf.Pow(1 - t, 3) * controlPoints[0].position + 3 * Mathf.Pow(1 - t, 2) * t * controlPoints[1].position + 3 * (1 - t) * Mathf.Pow(t, 2) * controlPoints[2].position + Mathf.Pow(t, 3) * controlPoints[3].position;
            Gizmos.DrawSphere(objectPostition, 0.25f);
        }
